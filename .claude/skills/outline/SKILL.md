...
- **広く浅く、重要概念だけ骨を通す**。網羅性は求めない。「あれも入れる、これも入れる」で章/セクションを盛らない。
- **「なんとなく理解できた」をゴールに**置く。完璧主義で挫折させない。深掘りしたい読者は AI に投げて深めてもらう前提でよい。
- 迷ったら**増やすのではなく削る**。カバレッジ確保のためのセクション増設は禁じ手。
- 「外せない骨は何か」を最初に決めてから章立てに入る。重要度の低いトピックは思い切ってカリキュラム外に置く判断をする。
...
- 章の合計が curriculum の `estimated_hours` とおおむね整合すること(ただし時間はチャプター/カリキュラムでは目安。ハード制約はセクション 3〜5 分のみ。詳細は PHILOSOPHY §3 / textbook-style §1)。
- セクション数は章あたり 2-7(目安 3-5)。**カバレッジ確保のためにセクション数を盛らない**。
