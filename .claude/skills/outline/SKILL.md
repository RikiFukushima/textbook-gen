...
3. **章立て案を提示**してユーザーレビューを受ける。
4. 各章を **12-25 分(目安 15 分)** の単位に分解し、その中で **3-5 セクション**に分割する。
5. 各セクションの `title` と `estimated_minutes`、扱う内容を提案する。
6. 書き出す:
   - 新規教材なら `textbooks/{slug}/meta.yaml`(`curriculum_order` 含む)
   - `textbooks/{slug}/curriculums/{curriculum-id}/curriculum.yaml`(title, order, estimated_hours, chapter_order, target_audience)
   - `textbooks/{slug}/curriculums/{curriculum-id}/outline.yaml`(骨子本体)
