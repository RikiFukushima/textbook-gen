...
- 入力 … シャッターボタンを押す(レンズが受け取る光も入力です)
- 処理 … 受け取った光を画像のデータに変える
- 出力 … 画面に写真を表示する

どの例も、形はちがっても「入力 → 処理 → 出力」の流れになっていることがわかります。電卓では指でボタンを押すことが入力でしたが、写真を撮るときはシャッターを押す操作やレンズが受け取る光が入力でした。このように、入力の形はさまざまですが、「外から何かを受け取っている」という点は共通しています。処理と出力も同じで、見た目はちがっても役割は変わりません。
