---
section_id: "03-02"
chapter_id: "03"
title: 非決定性をデバッグする — 再現できないバグと向き合う
order: 2
estimated_minutes: 4
estimated_chars: 1212
learning_points:
  - LLM の非決定性ゆえに同じ入力でも結果が変わることを理解する
  - 再現できないバグは「条件を固定して再現性を上げる」発想で攻める
  - トレースを起点に、どのステップで分岐したかを切り分ける
tags:
  - observability
  - debugging
  - nondeterminism
related_sections:
  - "03-01"
  - "03-03"
key_terms:
  - term: 非決定性
    definition: 同じ入力を与えても出力が毎回同じとは限らない性質。LLM はサンプリングで次の語を選ぶため、本質的に非決定な挙動を持つ。
  - term: 温度(temperature)
    definition: LLM の出力のばらつき具合を決めるパラメータ。低いほど決まった答えに寄り、高いほど多様な出力になる。
  - term: シードを固定する
    definition: 乱数の出発点をそろえて、サンプリングの揺れをできる限り再現できるようにする手法。
---

## このセクションで学ぶこと

- LLM の非決定性ゆえに同じ入力でも結果が変わることを理解する
- 再現できないバグは「条件を固定して再現性を上げる」発想で攻める
- トレースを起点に、どのステップで分岐したかを切り分ける

## 同じことをしても結果が変わる

ふつうのプログラムは、同じ入力なら同じ出力を返します。だからバグは「再現 → 観察 → 修正」の手順で潰せました。ところがエージェントの中心にいる LLM は**非決定的**です。次の語をサンプリング(確率的に選択)で決めるため、まったく同じプロンプトでも、あるときは正しいツールを呼び、あるときは別のツールを呼ぶ、ということが起こります。

非決定性は、実行ごとに結果がばらつく「揺れ」の話です。これとは別に、ai-agent-roadmap で触れたハルシネーションもエージェントを不安定にしますが、こちらは温度を 0 に下げても起こりうる別種の問題で、非決定性とイコールではありません(揺れを抑えても消えるとは限りません)。それでも「昨日は動いたのに今日は動かない」を引き当てたとき、コードを何度見返しても原因が見つからないのは、原因がコードではなく**モデルの揺れ**にあることが多いからです。

## 再現性を上げてから攻める

再現できないバグは、いきなり直そうとせず、まず**再現性を上げる**ことから始めます。揺れを生む要素を 1 つずつ固定していくのです。

```text
ばらつきを抑える主なつまみ
  temperature を下げる   → 出力が決まった方向に寄る
  シードを固定する        → サンプリングの揺れをそろえる
  入力(コンテキスト)を固定 → 同じ履歴・同じツール結果を渡す
```

温度を下げ、シードをそろえ、渡すコンテキストを固定すれば、揺れはかなり減ります。ただしシード指定は API によっては提供されておらず、あっても完全な再現を保証しない best-effort な扱いのことが多い点には注意してください。それでも再現するなら原因はプロンプトやツール設計にあり、固定したら消えるなら原因はモデルの揺れそのものです。この**切り分け**ができるだけで、打ち手は大きく変わります。

## トレースで分岐点を探す

再現性を上げたうえで頼るのが、前節のトレースです。失敗した実行と成功した実行のトレースを並べ、**どのステップから振る舞いが分かれたか**を探します。

```mermaid
sequenceDiagram
  participant U as ユーザー
  participant A as エージェント
  participant T as ツール
  U->>A: 同じ依頼
  A->>A: ステップ1 同じ判断
  A->>T: ステップ2 ここで引数が分岐
  T-->>A: 片方だけエラー
  A->>A: 以降の判断が連鎖的にずれる
```

多くの場合、最初の分岐は早い段階で起きていて、そこから連鎖的にずれていきます。最後の派手なエラーではなく、**一番上流の分岐点**を見つけることがデバッグの勘所です。

## 注意点

非決定性を完全に消すことはできませんし、消すべきでもありません。多様な出力こそが LLM の強みでもあるからです。狙うのは「ゼロにする」ことではなく、**原因を切り分けられるところまで揺れを抑える**ことです。本番では温度を上げて運用しつつ、デバッグ時だけ固定する、という使い分けが現実的です。

## まとめ

- LLM は非決定的なので、同じ入力でも結果が変わりうる。
- 再現できないバグは温度・シード・入力を固定し、再現性を上げてから攻める。
- 成功と失敗のトレースを並べ、連鎖の起点となる上流の分岐点を探す。
