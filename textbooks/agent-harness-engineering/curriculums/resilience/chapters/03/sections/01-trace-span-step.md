...
foundations で学んだ harness の 5 要素のうち「観測(Observability)」は、エージェントが何をしたかを開発者が外から見えるようにすることを指しました。この章で扱う観測は、まさにその**わたしたち開発者がエージェントを観測する**仕組みを掘り下げるものです。Claude Code がツール呼び出しを 1 件ずつ画面に出していたのも、人間向けの観測の一形態でした。なお、エージェント自身が環境の状態を見るのはループ内の「観測」ステップ(別概念)で、ここで扱う観測とは区別してください。
