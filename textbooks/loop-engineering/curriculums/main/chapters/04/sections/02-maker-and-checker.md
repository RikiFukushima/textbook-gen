...
learning_points:
  ...
  - カスタムコマンド /goal が maker と checker を分けている実例
tags:
  - maker-checker
  - サブエージェント
  - Claude Code
...
key_terms:
  ...
  - term: /goal
    definition: Boris Cherny が紹介したカスタムスラッシュコマンド。Claude Code の標準機能ではなく、検証可能な条件が満たされるまでエージェントを走らせ続け、別のモデルが完了したかを採点する仕組みの一例。
---
...
具体例として、Boris Cherny が紹介した `/goal` というカスタムスラッシュコマンドがあります(Claude Code に標準で備わったコマンドではなく、ユーザーが自作した一例です)。`/goal` は、検証可能な条件が満たされるまでエージェントを走らせ続け、その「完了したか」の採点を **別のモデル** に行わせます。
...
- カスタムコマンド /goal は別モデルが完了を採点する実例で、判定を分けるから「完了」が意味を持つ。
