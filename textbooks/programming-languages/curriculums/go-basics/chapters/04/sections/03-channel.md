...
channel は `make` で作成し、送受信には矢印型の **`<-` 演算子**を使います。まずは構文の形だけを示します。
...
これはあくまで各演算子の**書き方**を並べたものです。バッファのない channel では送信 `ch <- 42` が受信側の準備を待ってブロックするため、上の 3 行を 1 つの goroutine(たとえば `main`)でそのまま順に実行するとデッドロックします。実際には、次に見るように送信と受信を**別々の goroutine に分ける**のが基本です。
