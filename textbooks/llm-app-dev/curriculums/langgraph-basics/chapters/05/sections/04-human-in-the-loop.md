...
> 補足: 以下で使う `interrupt_before` + `invoke(None)` の静的中断は動くコードですが、現行の LangGraph が HITL で推奨するのは、ノード内で動的に `interrupt()` を呼んで承認データを受け取り、`Command(resume=...)` で再開するスタイルです(静的中断は主にデバッグ用途)。checkpointer に使う `MemorySaver` も、現行では `InMemorySaver` が正式名称です。ここでは仕組みを追いやすい静的中断で型を示します。
