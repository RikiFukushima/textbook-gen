...
- 並行ノード(§4-03 の fan-out)は **同じスーパーステップにまとめて数えられます**。ノードの総実行回数ではなくステップ数で数える点に注意してください。そのため fan-out で一度に多くのノードを走らせても、それ自体は 1 ステップとしてしか数えられません。上限に効いてくるのは主にループの反復回数のほうで、ループの回転が深いと `recursion_limit` に達しやすくなります。
