...
key_terms:
  ...
  - term: dotfiles
    definition: 「.」で始まる設定ファイル群の通称。.bashrc や .gitconfig などが代表例。
  - term: .bashrc
...
| `.gitconfig` | Git のユーザー名やエイリアスなど全体設定 |
...
graph TD
    ...
    H --> D["ls -a で見える dotfiles<br>.bashrc .gitconfig .ssh/"]
    D --> P[シェルやアプリの設定置き場]
```
...
- **`rm -r .*` のような削除は危険** です。`.*` はそのディレクトリにある `.bashrc` や `.ssh/` などすべての隠しファイルにまとめてマッチするため、意図しないものまで巻き込みます(なお、現代の `rm` は `.` と `..` 自体は明示的に拒否するので、親ディレクトリまで遡って消える心配はありません)。dotfiles を消すときは `.bashrc` のように名前を直接指定しましょう。
