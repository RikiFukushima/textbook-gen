...
前のセクションで触れたとおり、`man` のように画面全体が文書表示に切り替わるコマンドがあります。この状態(ページャ)では、打ったキーはページャへの操作(スクロールや検索など)として扱われ、シェルのコマンドとしては解釈されません。ここからの脱出は `Ctrl+C` ではなく **`q`(quit)を 1 回押す**です。すると元の画面に戻り、プロンプトが返ってきます。
