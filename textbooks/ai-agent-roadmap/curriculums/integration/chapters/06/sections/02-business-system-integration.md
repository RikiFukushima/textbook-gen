...
どのパターンを選ぶ場合も、OAuth 2.0 で取得するスコープは**業務上必要な最小集合**に絞ります。HubSpot なら「Contacts Read」だけで足りるユースケースに「Contacts Read/Write + Deals Read/Write」を要求しない、Google Workspace ならメールを読むだけで足りるなら書き込み権限まで含む `gmail.modify` ではなく `gmail.readonly` を選ぶ、といった具合です。Gmail のスコープは範囲の広い順に並んでおり、「やりたいことを満たす中で最も狭いもの」を都度選び直すのがコツです。
