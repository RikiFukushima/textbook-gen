...
ポイントは「評価結果を見てデータかハイパーパラメータに戻る」というループが必ず発生することです。最初から完璧な設定は当てられないので、**速く 1 周回せる仕組み** こそが品質を決めます。1 周が数時間かかる状態だと試行回数が稼げず、当てずっぽうの調整に陥りがちです。学習ログの保存先・評価スクリプト・採用基準をあらかじめ固定し、変えるのは「1 周ごとに 1 パラメータだけ」と決めておくと、どの変更が効いたかを切り分けられます。
