...
- **無限ループに備える**: ReAct は LLM の判断次第で無限に回り得ます。State に `iterations` を持たせて上限でルータが `"end"` を返す、invoke 時に `config={"recursion_limit": 25}` を渡すなどの安全策を **必ず** 入れてください(`recursion_limit` は compile ではなく実行時の config で指定します)
