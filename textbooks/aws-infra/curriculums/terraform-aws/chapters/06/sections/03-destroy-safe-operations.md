...
destroy も実行前に plan と同じ差分プレビューを表示し、`Do you really want to destroy all resources?` と確認を求めます。ここで `yes` と打つ前に、サマリの削除数と一覧を必ず読みます。とくに本番環境では、「テスト環境のつもりが本番の state を指していた」という取り違えが致命傷になります。実行前に `terraform workspace show`(workspace は同じ構成で開発・本番などの環境を分ける仕組みで、いまどの環境を選んでいるかを表示するコマンド)やバックエンド設定で、自分がどの環境を操作しているかを確認する癖をつけてください。
