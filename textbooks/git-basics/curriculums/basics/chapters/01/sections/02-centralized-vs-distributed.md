...
たとえば金曜の夜にサーバーがダウンすると、復旧するまでチーム全員が新しい変更を記録できません。出張中でネットワークが不安定なときに、過去のコミットを見返したいのに見られない、ということも起こります。履歴という大切な資産が、たった 1 か所のサーバーの状態に縛られてしまうわけです。これは規模が大きいチームほど深刻な問題になります。

## 分散型: 全員が完全な複製を持つ

こうした中央集約の弱点を解消するために生まれたのが、2 つめの**分散型バージョン管理**で、**Git** はこちらに属します。利用者は中央リポジトリを**クローン**し、履歴を含む完全な複製を手元に持ちます。手元に履歴がまるごとあるという点が、集中型との決定的な違いです。
