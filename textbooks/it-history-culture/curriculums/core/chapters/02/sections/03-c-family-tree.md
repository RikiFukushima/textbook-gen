...
key_terms:
  ...
  - term: C++
    definition: ビャーネ・ストロヴストルップが設計した C の拡張言語。1983 年に C++ という名前が付いた(名前自体は同僚 Rick Mascitti の提案)。「++」は C で「値を 1 増やす」演算子で、「C を 1 つ進めた言語」という言葉遊び
  - term: C#
...
C++ は、ビャーネ・ストロヴストルップが設計した拡張言語に 1983 年に付けられた名前です(名前自体は同僚 Rick Mascitti の提案でした)。「++」は C 言語で「値を 1 増やす」という意味の演算子。つまり C++ は「C を 1 つ進めた言語」という洒落です(当初の名前は素直に「C with Classes = クラス付き C」でした)。
