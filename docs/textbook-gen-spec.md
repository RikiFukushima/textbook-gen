...
> このドキュメントは**実装層**の正本である。プロダクトの判断基準・スタンスは `PHILOSOPHY.md`、執筆・レビューの規約は `.claude/skills/textbook-style/SKILL.md` に集約されている。両者と衝突した場合は **PHILOSOPHY が勝つ**。

## 0. 現在地と目的地

本書は**目的地**まで含めて記述する。読むときは現在地と切り分けること。

**目的地(将来):** 動画フィードを入口にして、テキストへ降りていく二層体験。読了履歴に基づく「再訪できる地図」UX。Supabase 上の進捗履歴。コミュニティ機能。RAG / チャットボット。

**現在地(現フェーズ):** **テキスト教材とクイズの生成パイプラインだけ**。動画レイヤーは未実装、ビューワも最低限。教材の量を増やし、精度を上げることが第一目的。マネタイズは未確定。

このため、§5(UX)・§6.6(Supabase)・§7(Claude Code skill 設計)・§8(ビューワ)に書かれた内容には、現在地で着手しているもの・目的地として記録しているだけのものが混在する。**実装に手をつける前に必ず PHILOSOPHY §0 で現フェーズのスコープを確認すること**。

詳細な思想・捨てるもの・成功指標は `PHILOSOPHY.md` を参照。
...
**時間が一次制約、字数は二次目安**(PHILOSOPHY §3): 上の表で**ハードに守るのは「推定読了時間 3-5 分」だけ**で、字数はそれを満たすための便宜的な目安にすぎない。日本語黙読 400-500 字/分(§4.2)の前提から逆算すると 3000 字超は時間オーバー側、コード多めセクションは字数が短くても写経で時間に達する。字数を埋めるために網羅を増やしたり、字数を短くするために本質を削ったりしない。詳細な運用ルールは `.claude/skills/textbook-style/SKILL.md` §4.3 を参照。
